...
# Routers
from app.routers import images, photos, auth, convert, vaults, voice, collab, gallery_assistant
...
# style transfer presets endpoint (server-side approximations)
try:
    from app.routers import style_transfer  # noqa: E402
    app.include_router(style_transfer.router)
except Exception as _ex:
    logger.warning(f"style_transfer router not available: {_ex}")
