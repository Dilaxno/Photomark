...
async def pricing_webhook(request: Request):
    ...
    try:
        ...
        logger.info(f"[pricing.webhook] secret configured={bool(secret_raw)}")
        if secret_raw:
            if secret_raw.startswith("whsec_"):
                try:
                    ...
                    logger.info(f"[pricing.webhook] using standardwebhooks verification; header id='{headers.get('webhook-id','')}', ts='{headers.get('webhook-timestamp','')}', sig_present={bool(headers.get('webhook-signature'))}")
                ...
                logger.info(f"[pricing.webhook] header secret provided={bool(secret_provided)}")
                if secret_provided != secret_raw:
                    logger.warning("[pricing.webhook] header secret mismatch")
                    return JSONResponse({"error": "Unauthorized"}, status_code=401)
    ...
    if payload is None:
        try:
            ...
            try:
                ...
                top_keys = []
            logger.info(f"[pricing.webhook] parsed JSON payload; top-level keys={top_keys}")
        except Exception as ex:
    ...
    if evt_type != "payment.succeeded":
        logger.info(f"[pricing.webhook] ignoring event type '{evt_type}' (expected 'payment.succeeded')")
    ...
    try:
        _obj_keys = list(obj.keys()) if isinstance(obj, dict) else []
        _meta_keys = list(meta.keys()) if isinstance(meta, dict) else []
    except Exception:
        _obj_keys, _meta_keys = [], []
    logger.info(f"[pricing.webhook] payload structure: obj_keys={_obj_keys} meta_keys={_meta_keys}")
    ...
    if not uid and obj:
        uid = str(obj.get("uid") or obj.get("firebase_uid") or "").strip()
    if not uid:
        uid = str(payload.get("uid") or payload.get("firebase_uid") or "").strip()

    if uid:
        logger.info(f"[pricing.webhook] resolved uid (priority chain): {uid}")

    # Fallback by email
    if not uid:
        em = _first_email_from_payload(payload) or _first_email_from_payload(obj or {})
        logger.info(f"[pricing.webhook] attempting email-based uid resolution; email='{em}'")
        if em:
            try:
                uid = get_uid_by_email(em) or ""
                logger.info(f"[pricing.webhook] resolved uid via email lookup: email='{em}' uid='{uid}'")
            except Exception as ex:
                logger.warning(f"[pricing.webhook] get_uid_by_email failed for email='{em}': {ex}")
                uid = ""

    if not uid:
        logger.warning("[pricing.webhook] missing uid in payment.succeeded payload; skipping upgrade")
        return {"ok": True, "skipped": True, "reason": "missing_uid"}

    # Resolve desired plan
    plan_raw = (
        str(meta.get("plan") or meta.get("tier") or meta.get("product_plan") or "").strip()
        or str(payload.get("plan") or "").strip()
        or str(obj.get("plan") or "").strip()
    )
    plan = _normalize_plan(plan_raw)
    if not plan:
        # Fallback to derive plan from products in the payload (Dodo product_cart)
    ...
    if not db or not fb_fs:
        logger.error(f"[pricing.webhook] Firestore unavailable; cannot persist plan (db_present={bool(db)} fb_fs_present={bool(fb_fs)})")
