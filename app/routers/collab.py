...
async def invite_member(
    ...
    accept_url = f"{front}#collab-login?accept_token={token}&owner_uid={owner_uid}"
...
@router.post("/accept")
async def accept_invite(request: Request, token: str = Body(..., embed=True), owner_uid: Optional[str] = Body(None, embed=True)):
    # Token-only acceptance (no Firebase auth required)
    index = read_json_key(_invite_index_key(token)) or {}
    owner_uid_idx = index.get("owner_uid")
    owner = owner_uid_idx or owner_uid
    if not owner:
        return JSONResponse({"error": "Invalid or expired token"}, status_code=400)

    team = _read_team(owner)
    ...
    if datetime.now(timezone.utc) > exp:
        try:
            ...
            _write_team(owner, team)
        except Exception:
            pass
        return JSONResponse({"error": "Invite expired"}, status_code=400)

    # Repair missing index if needed
    if not owner_uid_idx:
        try:
            write_json_key(_invite_index_key(token), {"owner_uid": owner, "token": token})
        except Exception:
            logger.warning("Failed to repair invite index for token")

    # Mark invite accepted but keep it until first successful collaborator login (per spec)
    inv["accepted"] = True
    _write_team(owner, team)

    return {"ok": True, "owner_uid": owner, "role": inv.get("role")}
