...
async def accept_invite(request: Request, token: str = Body(..., embed=True)):
    ...
    # Accept invite for any authenticated user; email may differ from the invitee.
    acc_email = (get_user_email_from_uid(uid) or "").lower()
    # Note: previously enforced acc_email == invite email; relaxed to support teammate invites.

    # Add member
    role = inv.get("role")
    member = {"uid": uid, "email": acc_email, "role": role, "joined_at": datetime.now(timezone.utc).isoformat()}
    # Replace existing membership by uid if any
    members = [m for m in team.get("members", []) if m.get("uid") != uid]
