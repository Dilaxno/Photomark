...
async def send_multiple_to_friend(
    ...
    """
    Send multiple images to a single friend's gallery. Each image can include an attached note.
    ...
    for idx, f in enumerate(files):
        try:
            ...
            # Optional per-item note JSON alongside image
            try:
                n = per_item_notes[idx] if idx < len(per_item_notes) else None
                if n and str(n).strip():
                    meta_key = f"{os.path.splitext(key)[0]}.json"
                    write_json_key(meta_key, {
                        "note": str(n).trim() if hasattr(str(n), 'trim') else str(n).strip(),
                        "from": get_user_email_from_uid(sender_uid) or None,
                        "at": _dt.utcnow().isoformat()
                    })
            except Exception as ex:
                logger.warning(f"collab: failed to write per-item note json for {key}: {ex}")
