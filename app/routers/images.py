...
async def images_upload(
    ...
    try:
        ...
        # 1) Save ORIGINAL under /originals
        ...
        key = f"users/{uid}/watermarked/{date_prefix}/{base}-{stamp}-txt-o{oext_token}.jpg"
...
@router.post("/images/detect-mine")
async def images_detect_mine(
    request: Request,
    file: UploadFile = File(...),
):
    """Check if an uploaded image contains an invisible signature matching the current user.
    Returns:
      - ok: boolean
      - embedded: whether any signature-like payload was detected
      - mine: whether it matches the current user's UID hash
    """
    eff_uid, req_uid = resolve_workspace_uid(request)
    if not eff_uid or not req_uid:
        return {"error": "Unauthorized"}
    ...
    try:
        ...
        return {"error": f"invalid image: {ex}"}

    try:
        payload = detect_signature(img, PAYLOAD_LEN)
        if payload is None:
            return {"ok": True, "embedded": False, "mine": False}
        is_mine = payload_matches_uid(payload, uid)
        # Mask payload in response for privacy
        hex_payload = payload.hex()
        masked = hex_payload[:8] + "…" + hex_payload[-8:]
        return {"ok": True, "embedded": True, "mine": bool(is_mine), "payload_preview": masked}
    except Exception as ex:
        logger.warning(f"detect-mine failed: {ex}")
