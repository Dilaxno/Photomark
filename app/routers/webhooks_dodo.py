...
async def dodo_webhook(
    ...
    if secret:
        try:
            ...
            provided_header = (x_dodo_signature or dodo_signature or "").strip()
            # Gather possible signature candidates from header
            candidates = []
            if "v1=" in provided_header or "sha256=" in provided_header:
                parts = [p.strip() for p in provided_header.split(",")]
                for p in parts:
                    if p.startswith("v1=") or p.startswith("sha256="):
                        candidates.append(p.split("=", 1)[1].strip())
            elif provided_header:
            ...
            ok = any(
                hmac.compare_digest(c.lower(), digest_hex)
