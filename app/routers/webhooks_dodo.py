...
async def dodo_webhook(request: Request, x_dodo_signature: Optional[str] = Header(default=None)):
    ...
    if secret:
        try:
            ...
            provided = (x_dodo_signature or "").strip()
            # Support possible formats: plain hex, or "t=..., v1=..."
            if "v1=" in provided:
                parts = [p.strip() for p in provided.split(",")]
                v1s = [p.split("=",1)[1] for p in parts if p.startswith("v1=")]
                provided_sig = (v1s[0] if v1s else "").lower()
            else:
                provided_sig = provided.lower()
