    ...
    if secret:
        try:
            ...
            if webhook_timestamp:
                message = f"{webhook_timestamp}.".encode("utf-8") + raw
            else:
                message = raw
            # Compute expected Base64 signature using HMAC-SHA256
            mac = hmac.new(secret.encode("utf-8"), message, hashlib.sha256).digest()
            expected_b64 = base64.b64encode(mac).decode().strip()
            ok = bool(received_sig) and hmac.compare_digest(received_sig, expected_b64)
            if not ok:
