...
async def dodo_webhook(
    ...
    for token in sigs:
        if "," not in token:
            continue
        prefix, sig = token.split(",", 1)
        if prefix.strip().lower() == "v1" and hmac.compare_digest(sig.strip(), computed_sig):
            valid = True
            break

    if not valid:
        logger.warning(
            f"[dodo.webhook] signature mismatch: rec={webhook_signature[:12]}..., exp={computed_sig[:12]}..., "
            f"ts='{webhook_timestamp}', id='{webhook_id}', raw_len={len(raw_body)}"
        )
        return JSONResponse({"error": "invalid signature"}, status_code=401)
