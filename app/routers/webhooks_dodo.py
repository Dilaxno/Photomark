...
from app.core.auth import get_uid_by_email, get_uid_from_request
...
async def dodo_webhook(
    ...
    if event_type in ("payment.succeeded", "payment_succeeded", "charge.succeeded"):
        ...
        # Prefer explicit metadata user_uid first, then fallback to known uid fields (avoid treating processor customer ids as our uid)
        meta_uid = str(
            (obj.get("metadata") or {}).get("user_uid")
            or (obj.get("metadata") or {}).get("uid")
            or (obj.get("metadata") or {}).get("firebase_uid")
            or (obj.get("metadata") or {}).get("userId")
            or (obj.get("metadata") or {}).get("user_id")
            or (data.get("metadata") or {}).get("user_uid")
            or (data.get("metadata") or {}).get("uid")
            or (data.get("metadata") or {}).get("firebase_uid")
            or (data.get("metadata") or {}).get("userId")
            or (data.get("metadata") or {}).get("user_id")
            or (payload.get("metadata") or {}).get("user_uid")
            or (payload.get("metadata") or {}).get("uid")
            or (payload.get("metadata") or {}).get("firebase_uid")
            or (payload.get("metadata") or {}).get("userId")
            or (payload.get("metadata") or {}).get("user_id")
            or ""
        ).strip()
        user_uid = meta_uid or str(
            obj.get("user_uid")
            or obj.get("uid")
            or obj.get("firebase_uid")
            or obj.get("userId")
            or obj.get("user_id")
            ...
            or data.get("user_id")
            or payload.get("uid")
            or payload.get("user_uid")
            or payload.get("userId")
            or payload.get("user_id")
            or ""
        ).strip()

        # Always extract external processor customer id separately
        _obj_customer = obj.get("customer") if isinstance(obj, dict) else None
        _data_customer = data.get("customer") if isinstance(data, dict) else None
        _payload_customer = payload.get("customer") if isinstance(payload, dict) else None
        cust_id = str(
            obj.get("customer_id")
            or obj.get("customerId")
            or (_obj_customer if isinstance(_obj_customer, str) else None)
            or ((_obj_customer or {}).get("id") if isinstance(_obj_customer, dict) else None)
            or data.get("customer_id")
            or data.get("customerId")
            or (_data_customer if isinstance(_data_customer, str) else None)
            or ((_data_customer or {}).get("id") if isinstance(_data_customer, dict) else None)
            or payload.get("customer_id")
            or payload.get("customerId")
            or (_payload_customer if isinstance(_payload_customer, str) else None)
            or ((_payload_customer or {}).get("id") if isinstance(_payload_customer, dict) else None)
            or ""
        ).strip()

        # Try customer-id mapping cache when uid is missing or looks like external id
        if (not user_uid) or user_uid.lower().startswith("cus_"):
            if cust_id:
                try:
                    ...
                    pass
        # If we already have a valid uid and a customer id, persist the mapping for future events
        elif cust_id and not user_uid.lower().startswith("cus_"):
            try:
                write_json_key(_custmap_key(cust_id), {"user_uid": user_uid, "mapped_at": datetime.utcnow().isoformat()})
            except Exception:
                pass

        # Email-based fallback disabled to avoid cross-account updates; require metadata user_uid or existing customer mapping
        if not user_uid or user_uid.lower().startswith("cus_"):
            logger.warning("[dodo.webhook] missing user id (metadata/customer mapping only); skip plan update")
            user_uid = ""

        # Identity guard not needed when email fallback is disabled
        ...
        if user_uid:
            ...
            try:
                ...
                # Derive purchased plan from metadata for entitlement mirror
                try:
                    _plan_meta = None
                    _m = (obj.get("metadata") or {}) if isinstance(obj, dict) else {}
                    _plan_meta = (_m.get("plan") or _m.get("tier") or _m.get("product_plan"))
                    if _plan_meta:
                        _plan_meta = str(_plan_meta).strip().lower()
                        if _plan_meta in ("photographer", "photographers"):
                            _plan_meta = "photographers"
                        elif _plan_meta in ("agency", "agencies"):
                            _plan_meta = "agencies"
                except Exception:
                    _plan_meta = None
...
# Create Dodo checkout with enforced metadata (user_uid and plan)
@router.post("/checkout")
async def dodo_create_checkout(request: Request, payload: dict = Body(...)):
    uid = get_uid_from_request(request)
    if not uid:
        # Attempt to resolve by email from payload/metadata/customer
        email_fallback = str(
        ...
        if email_fallback:
            try:
                uid = get_uid_by_email(email_fallback)
            except Exception:
                uid = None
    if not uid:
        return JSONResponse({"error": "Unauthorized"}, status_code=401)

    if not DODO_API_BASE or not DODO_CHECKOUT_PATH or not DODO_API_KEY:
        logger.error("[dodo.checkout] Dodo API not configured")
        return JSONResponse({"error": "payments unavailable"}, status_code=500)

    # Normalize plan from client payload (optional, default to 'pro')
    plan = str(((payload or {}).get("plan") or (payload or {}).get("tier") or (payload or {}).get("product_plan") or "")).strip().lower()
    if plan in ("photographer", "photographers"):
        plan = "photographers"
    elif plan in ("agency", "agencies"):
        plan = "agencies"
    if not plan:
        plan = "pro"

    # Ensure metadata includes user_uid and plan; also include user email if available
    out_payload = dict(payload or {})
    meta = out_payload.get("metadata") if isinstance(out_payload.get("metadata"), dict) else {}
    # Fetch current plan from Firestore for auditing and add robust uid aliases
    current_plan = "free"
    try:
        _fs = _get_fs_client()
        if _fs and uid:
    ...
    meta.update({"user_uid": uid, "uid": uid, "firebase_uid": uid, "plan": plan, "current_plan": current_plan})

    try:
        # Optional: include purchaser email for auditing (not used for UID resolution)
        from app.core.auth import get_user_email_from_uid as _user_email
        email = (_user_email(uid) or "").strip()
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=20) as resp:
            ...
            try:
                ...
                return JSONResponse({"ok": True, "raw": body.decode("utf-8", "ignore")})
    except urllib.error.HTTPError as ex:
        err_body = ex.read().decode("utf-8", "ignore")
        logger.warning(f"[dodo.checkout] HTTP {ex.code}: {err_body}")
        try:
            return JSONResponse(json.loads(err_body), status_code=ex.code)
        except Exception:
            return JSONResponse({"error": "checkout failed", "detail": err_body}, status_code=ex.code)
    except Exception as ex:
