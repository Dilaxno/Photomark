...
async def dodo_webhook(
    ...
    if secret:
        try:
            ...
            # Normalize header and collect candidate signatures
            candidates: list[str] = []
            if provided_header:
                parts = [p.strip() for p in provided_header.split(",") if p.strip()]
                if parts:
                    for p in parts:
                        # Accept formats: "v1=...", "sha256=...", "v1", "<sig>"
                        if "=" in p:
                            k, v = p.split("=", 1)
                            k = k.strip().lower()
                            v = v.strip()
                            if k in ("v1", "sha256") and v:
                                candidates.append(v)
                        else:
                            # If it's just the signature or a version marker, keep signature-like tokens
                            if p.lower() in ("v1", "sha256", "t", "ts", "timestamp"):
                                continue
                            candidates.append(p)
                    # Fallback: if header is exactly like "v1,<sig>", last token is the signature
                    if len(candidates) == 0 and len(parts) >= 2:
                        candidates.append(parts[-1])
                else:
                    candidates.append(provided_header)
            # Compute both hex and base64 digests from raw body
            mac = hmac.new(secret.encode("utf-8"), raw, hashlib.sha256)
            digest_hex = mac.hexdigest()
            digest_b64 = base64.b64encode(mac.digest()).decode().strip()
            ok = any(
                (c and (hmac.compare_digest(c.lower(), digest_hex)
                        or hmac.compare_digest(c, digest_b64)
                        or hmac.compare_digest(c.lower(), digest_b64.lower())))
                for c in candidates
            )
            if not ok:
                logger.warning(f"[dodo.webhook] signature verification failed; header='{provided_header}'")
