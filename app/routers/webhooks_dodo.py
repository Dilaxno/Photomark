...
async def dodo_webhook(
    ...
    if event_type in ("payment.succeeded", "payment_succeeded", "charge.succeeded"):
        ...
        # Fallback: resolve uid by email when still unresolved
        if not user_uid or user_uid.lower().startswith("cus_"):
            # Prefer client-sent metadata email over provider/customer email to avoid misattribution
            email_guess = str(
        ...
        # Identity guard: if metadata provides user_uid and metadata.email maps to a different uid, skip updating
        try:
            if meta_uid and (locals().get("email_guess") or None):
                from app.core.auth import get_uid_by_email as _uid_by_email_check
                uid_from_email = _uid_by_email_check(locals().get("email_guess")) if locals().get("email_guess") else None
                if uid_from_email and uid_from_email != meta_uid:
                    logger.warning(f"[dodo.webhook] metadata uid/email mismatch: meta_uid={meta_uid} email={locals().get('email_guess')} -> {uid_from_email}; skipping plan update")
                    return {"ok": True}
        except Exception:
            pass
