            ...
            if provided_header:
                ...
                if len(parts) >= 2 and parts[0].lower() in ("v1", "sha256") and "=" not in parts[0]:
                    ...
                    if not received_sig and parts:
                        ...
                        received_sig = parts[-1]
            # Build message "timestamp.payload" if timestamp is provided, else raw payload only
            # Some providers include id as well -> try variants for robustness
            candidates_messages = []
            if webhook_timestamp:
                candidates_messages.append(f"{webhook_timestamp}.".encode("utf-8") + raw)
                if webhook_id:
                    candidates_messages.append(f"{webhook_timestamp}.{webhook_id}.".encode("utf-8") + raw)
            candidates_messages.append(raw)

            # Compute expected Base64 signature using HMAC-SHA256, try all candidate message formats
            expected_any = []
            for m in candidates_messages:
                mac = hmac.new(secret.encode("utf-8"), m, hashlib.sha256).digest()
                expected_any.append(base64.b64encode(mac).decode().strip())
            ok = bool(received_sig) and any(hmac.compare_digest(received_sig, e) for e in expected_any)
            if not ok:
                logger.warning(f"[dodo.webhook] signature verification failed; hdr='{provided_header}', ts='{webhook_timestamp}'")
