...
# Allowed plans utils
_ALLOWED_PLANS_CACHE = {"ts": 0, "plans": set()}

def _to_plan_slug(s: Optional[str]) -> str:
    p = (s or "").strip().lower()
    if p in ("photographer", "photographers", "photo", "photos", "pg", "p", "photography"):
        return "photographers"
    if p in ("agency", "agencies", "agencys", "a", "ag"):
        return "agencies"
    return ""

def _fetch_plans_from_pricing() -> set[str]:
    plans: set[str] = set()
    try:
        ...
        for path in ("/api/pricing.json", "/pricing.json", "/pricing"):
            url = origin + path
            try:
                req = urllib.request.Request(url, headers={"User-Agent": os.getenv("DODO_HTTP_USER_AGENT","Mozilla/5.0")})
                with urllib.request.urlopen(req, timeout=2) as resp:
                    body = resp.read().decode("utf-8","ignore")
                    low = body.lower()
                    if "photographer" in low:
                        plans.add("photographers")
                    if "agenc" in low:
                        plans.add("agencies")
                    if plans:
                        return plans
...
def _allowed_plans() -> set[str]:
    ...
    now = int(time.time())
    if _ALLOWED_PLANS_CACHE["plans"] and now - int(_ALLOWED_PLANS_CACHE["ts"]) < 600:
        return set(_ALLOWED_PLANS_CACHE["plans"])

    plans: set[str] = set()
    try:
        raw = (os.getenv("DODO_ALLOWED_PLANS") or os.getenv("ALLOWED_PLANS") or os.getenv("PRICING_PLANS") or "")
        if raw:
            for tok in raw.split(","):
                slug = _to_plan_slug(tok)
                if slug:
                    plans.add(slug)
    except Exception:
        pass
    ...
    if not plans:
        plans = _fetch_plans_from_pricing()

    if not plans:
        plans = {"photographers", "agencies"}

    _ALLOWED_PLANS_CACHE["plans"] = plans
    _ALLOWED_PLANS_CACHE["ts"] = now
    return plans

def _normalize_plan(plan: Optional[str]) -> str:
    slug = _to_plan_slug(plan)
    return slug if slug and slug in _allowed_plans() else ""
...
async def dodo_webhook(
    ...
    # Cross-validate UID using Firebase Auth via email and expose primary email for logs
    primary_email = ""
    auth_uid = None
    try:
        email_candidates = []
        ...
        for k in ("email","customer_email","payer_email","buyer_email"):
            ...
            if isinstance(v, str) and "@" in v:
                email_candidates.append(v.strip().lower())
        if email_candidates:
            primary_email = email_candidates[0]
        if firebase_enabled and fb_auth and primary_email:
            try:
                _auth_user = fb_auth.get_user_by_email(primary_email)
                auth_uid = getattr(_auth_user, "uid", None)
            except Exception:
                auth_uid = None
    except Exception:
        pass

    if auth_uid and uid and uid != auth_uid:
        logger.warning(f"[dodo.webhook] uid_mismatch: metadata_uid={uid} auth_uid={auth_uid} email={primary_email}; using auth_uid")
        uid = auth_uid
    elif not uid and auth_uid:
        uid = auth_uid

    plan = _derive_plan_from_payload(obj, meta) or _normalize_plan(meta.get("plan") or meta.get("tier") or meta.get("product_plan"))

    # Firestore user snapshot for traceability
    try:
        _db_ro = _get_fs_client()
        if _db_ro and fb_fs:
            _snap_user = _db_ro.collection("users").document(uid).get()
            if getattr(_snap_user, "exists", False):
                _ud = _snap_user.to_dict() or {}
                logger.info(f"[dodo.webhook] firestore_user: uid={uid} email={_ud.get('email','')} current_plan={_ud.get('plan','')} isPaid={bool(_ud.get('isPaid') or False)}")
    except Exception as _ex:
        logger.debug(f"[dodo.webhook] firestore user lookup failed for {uid}: {_ex}")

    # Enforce allowlist from env/pricing
    _plans = _allowed_plans()
    if plan not in _plans:
        raw_plan = str(meta.get("plan") or meta.get("tier") or meta.get("product_plan") or plan or "").strip()
        logger.warning(f"[dodo.webhook] unknown or unsupported plan '{raw_plan}' for uid={uid} email={primary_email} auth_uid={auth_uid or ''}; allowed={sorted(list(_plans))}; skipping persist")
        return {"ok": True}
...
async def dodo_create_checkout(request: Request, payload: dict = Body(...)):
    ...
    if not plan:
        return JSONResponse({"error": f"invalid plan; allowed: {', '.join(sorted(_allowed_plans()))}"}, status_code=400)
