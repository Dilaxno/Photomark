            ...
            if provided_header:
                ...
                if len(parts) >= 2 and parts[0].lower() in ("v1", "sha256") and "=" not in parts[0]:
                    ...
                    if not received_sig and parts:
                        ...
                        received_sig = parts[-1]
            # Strict Dodo scheme: Base64(HMAC_SHA256(secret, "{timestamp}.{raw_body}"))
            if not webhook_timestamp:
                logger.warning("[dodo.webhook] missing webhook-timestamp header")
                return JSONResponse({"error": "invalid signature"}, status_code=401)

            # Build exact message bytes: timestamp + '.' + raw body bytes
            message = webhook_timestamp.encode("utf-8") + b"." + raw
