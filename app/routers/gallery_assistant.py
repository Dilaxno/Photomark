...
async def voice_command(request: Request, payload: Dict[str, Any] = Body(...)):
    # Accepts { text: string }, forwards to /chat using the same flow
    text = str((payload or {}).get("text") or "").strip()
    if not text:
        return JSONResponse({"error": "text required"}, status_code=400)
    # Reuse chat with a single user message
    return await chat(request, {"messages": [{"role": "user", "content": text}]})
