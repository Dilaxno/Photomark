...
# --------------------------------
# FastAPI Router (public endpoints)
# --------------------------------
...
async def api_simulate(
    ...
    focus_distance_m: Optional[float] = Form(2.0),
    ...
    try:
        ...
        result = process_simulation(
            ...
            focus_distance_m=focus_distance_m if focus_distance_m is not None else 2.0,
            vignetting=vignetting,
            chromatic_aberration=chromatic_aberration,
            distortion_k1=distortion_k1,
            bokeh_strength=bokeh_strength,
        ...
        # Base64-encode image payloads for JSON transport
        ...
        for k, v in images.items():
            if v is None:
                ...
                encoded[k] = base64.b64encode(v).decode("ascii")
        result["images"] = encoded
        return result
    except HTTPException:
        raise
    except Exception as ex:
        raise HTTPException(status_code=400, detail={"error": f"Simulation failed: {ex}"})