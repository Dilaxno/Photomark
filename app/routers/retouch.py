...
async def ai_analyze(
    ...
    """
    Deprecated: This endpoint is disabled. Use /api/retouch/hf-apply to apply edits directly with Hugging Face.
    """
    raise HTTPException(status_code=410, detail="Deprecated: use /api/retouch/hf-apply")
...
async def hf_apply(
    ...
    try:
        ...
        if resp.status_code == 404:
            logger.error("HF endpoint 404 — model may not be available on public Inference API")
            raise HTTPException(status_code=502, detail="Model not available on public Inference API. Please provision a dedicated Inference Endpoint or use a supported image-to-image model.")
