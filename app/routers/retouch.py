...
def preprocess_image(im: np.ndarray, model_input_size: list) -> torch.Tensor:
    if im.ndim < 3:
        im = im[:, :, np.newaxis]
    im_tensor = torch.tensor(im, dtype=torch.float32).permute(2, 0, 1)  # HWC -> CHW
...
async def background_replace(
    ...
    replace_with: Optional[str] = Form(None),  # hex color like #000000 or keyword 'blurred'
    ...
    try:
        ...
        # Run RMBG model (direct HF model)
        ...
        # Try to obtain the predicted alpha/matte tensor
        pred = None
        ...
        if background_url:
            ...
            if rv.lower() == 'blurred':
                # For 'blurred' we return transparent PNG and let frontend handle CSS blur background
                out = cut
            else:
                try:
        ...
        # Store with cache-busting unique key to avoid CDN/browser caching issues
        ...
        # Add query param as extra cache-buster for good measure
