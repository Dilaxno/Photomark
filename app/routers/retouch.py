...
_rmbg_device = None
_vignette_cache = {}
_rmbg_mask_cache = {}  # Cache RMBG masks keyed by file hash
...
def get_rmbg_model():
    ...
    if _rmbg_model is None:
        ...
        try:
            ...
            logger.exception(f"Failed to load RMBG model: {e}")
            # Optional: raise HTTPException to stop processing
            raise RuntimeError("RMBG model failed to load")
...
# ---------------- RMBG MASK CACHING ----------------
def compute_rmbg_mask(img: Image.Image) -> Image.Image:
    ...
    orig_im = np.array(img.convert("RGB"))
...
async def background_replace(
    ...
    try:
        ...
        if background_url:
            ...
            rgb = ImageColor.getrgb(replace_with.strip())
