...
def compute_mask(img: Image.Image) -> Image.Image:
    ...
    try:
        ...
        if not isinstance(fg, Image.Image):
            fg = Image.open(io.BytesIO(fg))  # handle bytes return just in case
        if fg.mode != "RGBA":
            fg = fg.convert("RGBA")
        # Check if any transparency was introduced; if none, treat as failure
        ...
        # Re-raise so callers can return an error rather than silently failing
        raise
...
async def background_replace(
    ...
    try:
        ...
        # Save to storage and return a proxy URL (same origin) to avoid external CORS
        url = upload_bytes(key, buf.getvalue(), content_type="image/png")
        # If stored locally, url will be /static/..., already same-origin
        # If using R2/public URL, we still offer a proxy for same-origin fetching
        proxy_url = f"/api/retouch/proxy?key={key}&v={suffix}"
        return {"ok": True, "url": proxy_url, "key": key}
...
async def proxy_image(key: str):
    """Serve stored image via same-origin to avoid external CORS issues."""
    try:
        if s3 and R2_BUCKET:
        ...
        logger.warning(f"proxy fetch failed for {key}: {ex}")
        return Response(status_code=404)
