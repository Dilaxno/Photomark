...
async def ai_retouch_instruct_pix2pix(
    ...
    try:
        ...
        # Import Qwen pipeline when available (newer diffusers), ignore if not
        try:
            from diffusers import QwenImageEditPipeline  # type: ignore
            _HAS_QWEN_PIPE = True
        except Exception:
            _HAS_QWEN_PIPE = False
        ...
        if not hasattr(ai_retouch_instruct_pix2pix, "_pipe") or ai_retouch_instruct_pix2pix._pipe is None:
            ...
            # Prefer native pipeline if available; fallback to generic with remote code
            if _HAS_QWEN_PIPE:
