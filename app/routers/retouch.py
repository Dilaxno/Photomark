...
async def remove_background(
    ...
    """Remove background using rembg. If hex_color provided, composite subject on that color, else return transparent PNG."""
    raw = await file.read()
    if not raw:
    ...
    try:
        ...
        # If a color is provided, composite onto solid color background
        if hex_color:
            c = hex_color.strip()
            if c.startswith('#'):
                c = c[1:]
            ...
            if len(c) != 6:
                return {"error": "hex_color must be in #RRGGBB or #RGB format"}
            try:
                r = int(c[0:2], 16)
                g = int(c[2:4], 16)
                b = int(c[4:6], 16)
            except ValueError:
                return {"error": "Invalid hex_color"}
