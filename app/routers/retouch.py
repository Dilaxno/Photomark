...
async def ai_retouch_instruct_pix2pix(
    ...
    """
    Calls Hugging Face Inference API to apply text-guided edits to the uploaded image
    using timbrooks/instruct-pix2pix. Returns stored image URL.
    """
    token = os.getenv("HUGGING_FACE_API_TOKEN", "").strip()
    if not token:
        return {"error": "HUGGING_FACE_API_TOKEN not configured on server"}

    raw = await file.read()
    if not raw:
        return {"error": "empty file"}

    # Prepare multipart form for HF Inference API
    # We request image output directly
    headers = {
    ...
    data = {
        "inputs": instruction,
        # parameters must be JSON-encoded when using multipart
    ...
    files = {
        "image": (file.filename or "image.png", raw, file.content_type or "image/png"),
    }
    ...
    try:
        ...
        if resp.status_code == 503:
            # Model cold start; return message so client can retry later
            try:
                info = resp.json()
            except Exception:
                info = {"error": resp.text}
            return {"error": "Model is loading, try again soon", "detail": info}
        if not resp.is_success:
            try:
                err = resp.json()
            except Exception:
                err = {"error": resp.text}
            logger.warning(f"HF instruct-pix2pix error: {err}")
            return {"error": err.get("error") or err}

        # Expecting image bytes
        if not ct.startswith("image/"):
            # Some backends return an array of objects [{"generated_image": "..."}] or similar
            try:
                data_json = resp.json()
            except Exception:
                data_json = None
            return {"error": "Unexpected response from HF API", "detail": data_json}
        ...
        logger.exception(f"HF instruct-pix2pix failed: {ex}")
        return {"error": str(ex)}
