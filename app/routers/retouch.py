...
def _load_lut_from_r2(lut_key: str):
    ...
    if s3 is None:
        raise RuntimeError('R2/S3 not configured')
    ...
    try:
        ...
        logger.exception(f'Failed to load LUT {lut_key} from R2: {e}')
        raise


@router.get('/luts')
async def list_luts():
    """List available .cube LUT keys from the R2 bucket (defaults to 'luts')."""
    try:
        if s3 is None:
            return []
        bucket_name = R2_BUCKET or 'luts'
