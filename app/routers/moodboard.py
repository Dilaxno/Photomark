...
async def generate_moodboard(request: Request, files: List[UploadFile] = File(...)):
    ...
    # Save per-image pages for PDF export (one image per PDF page)
    ...
    for idx, src in enumerate(file_paths, start=1):
        try:
            ...
            # Optional clamp to avoid gigantic single pages
            ...
            if scale < 1.0:
                try:
                    from PIL import Image as _PILImage
                    im = im.resize((max(1, int(w * scale)), max(1, int(h * scale))), resample=_PILImage.Resampling.LANCZOS)
                except Exception:
                    im = im.resize((max(1, int(w * scale)), max(1, int(h * scale))))
            im.save(os.path.join(pages_dir, f"{idx:03d}.jpg"), format="JPEG", quality=95, subsampling=0, optimize=True)
        except Exception:
