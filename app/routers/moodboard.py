...
def build_moodboard_collage(images: List[Image.Image], palette: List[str]) -> bytes:
    """Generate one big mosaic image (no side panel, no palette), puzzle-like grid.
    - Up to 10 images arranged in a 5x2 grid (responsive cell size)
    - Images are thumbnailed and centered into rounded cells
    """
    # Canvas size tuned for a single large mosaic
    W, H = 2000, 1200
    ...
    # Grid layout
    cols, rows = 5, 2
    pad = 14  # slightly larger padding for a cleaner puzzle feel
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            draw.rounded_rectangle([x, y, x + cell_w, y + cell_h], radius=18, fill=(32, 32, 36))
            canvas.paste(im_copy, (ox, oy))
            i += 1

    # Export mosaic only (no palette, no sidebars)
    buf = io.BytesIO()
