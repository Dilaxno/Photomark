...
async def vaults_remove(request: Request, vault: str = Body(..., embed=True), keys: List[str] = Body(..., embed=True), password: Optional[str] = Body(None, embed=True), delete_from_r2: Optional[bool] = Body(False, embed=True)):
    ...
    try:
        ...
        if delete_from_r2 and to_remove:
            ...
            # Purge from all vaults to avoid reappearing links
            try:
                prefix = f"users/{uid}/vaults/"
                if s3 and R2_BUCKET:
                    bucket = s3.Bucket(R2_BUCKET)
                    for obj in bucket.objects.filter(Prefix=prefix):
                        ...
                        if not keys_list:
                            continue
                        remain_keys = [k for k in keys_list if k not in to_remove]
                        if remain_keys != keys_list:
                            write_json_key(vkey, {"keys": sorted(set(remain_keys))})
                else:
                    base = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "static"))
                    dir_path = os.path.join(base, prefix)
                    if os.path.isdir(dir_path):
                        for f in os.listdir(dir_path):
                            ...
                            rel_key = os.path.relpath(vpath, base).replace('\\', '/')
                            ...
                            remain_keys = [k for k in keys_list if k not in to_remove]
                            if remain_keys != keys_list:
                                write_json_key(rel_key, {"keys": sorted(set(remain_keys))})
            except Exception as ex:
                logger.warning(f"Failed to purge vault references: {ex}")

        return {"vault": _vault_key(uid, vault)[1], "count": len(remain), "deleted": deleted, "errors": errors}
    except Exception as ex:
        return JSONResponse({"error": str(ex)}, status_code=400)
