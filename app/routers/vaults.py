...
from app.core.config import s3, R2_BUCKET, R2_PUBLIC_BASE_URL, logger, DODO_API_BASE, DODO_CHECKOUT_PATH, DODO_PRODUCTS_PATH, DODO_API_KEY, DODO_WEBHOOK_SECRET
...
async def vaults_shared_checkout(payload: CheckoutPayload, request: Request):
    ...
    # Create a product or use cart for new Checkout Sessions API
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            # New unified checkout sessions endpoint expects product_cart
            # We'll create an ephemeral product representing this vault license if needed.
            # Minimal product: one-time price in cents with currency.
            product_payload = {
                "name": f"Vault License: {vault}",
                "price": {
                    "type": "one_time_price",
                    "price": amount,
                    "currency": currency,
                ...
                "metadata": {"uid": uid, "vault": vault}
            }
            prod_url = f"{DODO_API_BASE}{DODO_PRODUCTS_PATH}"
            ...
            prod_text = getattr(prod_resp, 'text', '')
            try:
                prod_data = prod_resp.json()
            except Exception:
                prod_data = None
            if prod_resp.status_code >= 400 or not isinstance(prod_data, dict):
                raise RuntimeError(f"product create failed ({prod_resp.status_code}): {prod_text[:300]}")
            product_id = prod_data.get("product_id")
            if not product_id:
                raise RuntimeError("product_id missing in response")

            # Create checkout session using product_cart
        ...
        # Try to parse JSON safely and capture response text for debugging
        ...
        if resp is None or resp.status_code >= 400:
            ...
            raise RuntimeError(f"checkout failed ({getattr(resp,'status_code',0)}): {err_detail}")
