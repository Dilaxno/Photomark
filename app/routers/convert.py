...
def letterbox_embed_raw(raw: bytes, ratio_str: str, bg_hex: str = '#000000') -> Optional[bytes]:
    ...
    with WandImage(blob=raw) as img:
        ...
        if abs(cur_ar - ar) < 1e-6:
            ...
            left, top = 0, max(0, (target_h - h) // 2)
        else:
            # too tall -> increase width
            target_h = h
            target_w = int(round(h * ar))
            left, top = max(0, (target_w - w) // 2), 0

        with WandImage(width=target_w, height=target_h, background=Color(bg_hex or '#000000')) as canvas:
            # Composite original centered on background canvas
            try:
                # Ensure alpha preserved if any
                img.alpha_channel = True
            except Exception:
                pass
            canvas.composite(img, left=left, top=top)
            return canvas.make_blob('png')
