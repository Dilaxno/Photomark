...
try:
    from app.core.auth import firebase_enabled  # ensure admin is initialized
...
async def affiliates_track_signup(payload: dict = Body(...)):
    ...
    try:
        ...
        # Mirror attribution in Firestore for analytics if available
        try:
            if _fs_client:
                _fs_client.collection('affiliate_attributions').document(new_user_uid).set({
                    'affiliate_uid': affiliate_uid,
