...
async def create_pricing_link(request: Request):
    ...
    # Build payload using common Dodo structures observed in webhooks
    payload = {
        "metadata": {
            "user_uid": uid,
            "plan": plan,
    ...
    # Add customer email if available (helps with receipts and idempotency on provider side)
    email = _get_user_email(uid)
    if email:
        payload["customer"] = {"email": email}

    # Try common endpoints: prefer payment-links; fallback to checkout/session
    endpoints = [
