...
async def api_photos_external(request: Request):
    eff_uid, req_uid = resolve_workspace_uid(request)
    if not eff_uid or not req_uid:
        return JSONResponse({"error": "Unauthorized"}, status_code=401)
    if not has_role_access(req_uid, eff_uid, 'gallery'):
    ...
    if s3 and R2_BUCKET:
        try:
            ...
            for obj in bucket.objects.filter(Prefix=prefix):
                ...
                # Try to derive corresponding original key from naming convention
                original_key = None
                if "-o" in name:
                    try:
                        base_part = name.rsplit("-o", 1)[0]
                        ...
                        if m:
                            ...
                            if os.path.isdir(wm_dir):
                                for wf in os.listdir(wm_dir):
                                    if wf.startswith(f"{base_part}-{stamp}-") and '-fromfriend' in wf:
