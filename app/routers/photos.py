...
async def api_photos_delete(request: Request, keys: List[str] = Body(..., embed=True)):
    ...
    if s3 and R2_BUCKET:
        try:
            ...
            # Include potential sidecar JSON files next to each allowed key
            sidecars = [os.path.splitext(k)[0] + ".json" for k in allowed]
            to_delete = sorted(set(allowed + sidecars))
        ...
        for k in keys:
            ...
            # Try to remove sidecar JSON as well
            try:
                sidecar_key = os.path.splitext(k)[0] + ".json"
                sidecar_path = os.path.join(static_dir, sidecar_key)
                if os.path.exists(sidecar_path):
                    os.remove(sidecar_path)
                    deleted.append(sidecar_key)
            except Exception as ex:
                errors.append(f"{sidecar_key}: {ex}")
