...
async def api_photos_delete(request: Request, keys: List[str] = Body(..., embed=True)):
    ...
    # 1) Delete underlying objects from R2/local
    if s3 and R2_BUCKET:
        try:
            bucket = s3.Bucket(R2_BUCKET)
...
async def api_photos_download_zip(request: Request, body: dict = Body(...)):
    eff_uid, req_uid = resolve_workspace_uid(request)
    if not eff_uid or not req_uid:
        return JSONResponse({"error": "Unauthorized"}, status_code=401)
    if not has_role_access(req_uid, eff_uid, 'gallery'):
    ...
    safe = [str(k).lstrip('/') for k in keys if str(k).startswith(f"users/{uid}/")]
    if not safe:
    ...
    return StreamingResponse(iter([mem.getvalue()]), media_type="application/zip", headers=headers)
