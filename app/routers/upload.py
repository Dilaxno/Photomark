...
async def upload(
    ...
    signature_bytes = await signature.read() if signature is not None else None
    use_signature = bool(signature_bytes)

    # Validate text mode
    if not use_signature and not (watermark or '').strip():
        return JSONResponse({"error": "watermark text required or provide signature"}, status_code=400)
    ...
    for uf in files:
        try:
            ...
            if use_signature:
                sig = Image.open(io.BytesIO(signature_bytes)).convert("RGBA")  # type: ignore[arg-type]
            ...
            suffix = 'sig' if use_signature else 'txt'
