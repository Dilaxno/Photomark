...
def add_text_watermark_tiled(
    ...
    if not HAS_FT:
        # PIL-based tiling path (no warning spam)
        ...
        global TTF_PATH
        try:
            font = ImageFont.truetype((TTF_PATH or 'DejaVuSans.ttf'), size)
        except Exception:
            font = ImageFont.load_default()

        # Build unit tile
        # First measure text box
        dummy_img = Image.new('RGBA', (size * 10, size * 3), (0, 0, 0, 0))
        ...
        unit_w = tw + pad_x
        unit_h = th + pad_y
        unit = Image.new('RGBA', (max(1, unit_w), max(1, unit_h)), (0, 0, 0, 0))
        ud = ImageDraw.Draw(unit)
        bx = max(1, size // 10)
        by = max(1, size // 10)
        # Shadow
        shadow_offset = max(1, size // 10)
        ud.text((bx + shadow_offset, by + shadow_offset), text, font=font, fill=(0, 0, 0, int(min(200, a * 255))))
        # Stroke + main text
        stroke_w = max(1, size // 14)
        ud.text((bx, by), text, font=font, fill=(r, g, b, int(a * 255)), stroke_width=stroke_w, stroke_fill=(0, 0, 0, int(min(220, a * 255))))

        # Tiling
        gap = max(8, int(min(unit.width, unit.height) * max(0.05, min(1.0, spacing_rel or 0.3))))
        step_x = unit.width + gap
        step_y = unit.height + gap

        big = Image.new('RGBA', (w * 3, h * 3), (0, 0, 0, 0))
        for y0 in range(0, big.height, step_y):
            for x0 in range(0, big.width, step_x):
                big.alpha_composite(unit, dest=(x0, y0))
