...
from app.utils.storage import read_json_key
...
def resolve_workspace_uid(request: Request) -> tuple[Optional[str], Optional[str]]:
    """Return (effective_uid, requester_uid).
    - If the requester is a collaborator (synthetic uid: "collab:{owner_uid}:{email}"),
      use the owner's workspace uid.
    - Otherwise, try owner pointer mapping, falling back to requester's uid.
    """
    req_uid = get_uid_from_request(request)
    if not req_uid:
        return None, None

    # Collaborator JWTs carry a synthetic uid that encodes the owner uid.
    owner_from_collab, _ = _parse_collab_uid(req_uid)
    if owner_from_collab:
        return owner_from_collab, req_uid
...
try:
    ...
    logger.info("Firebase Admin initialized (mod)")
except Exception as ex:
...
def get_uid_from_request(request: Request) -> Optional[str]:
    ...
    try:
        from app.core.config import COLLAB_JWT_SECRET
