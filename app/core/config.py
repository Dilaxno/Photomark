...
# LUT previews storage (optionally a separate bucket/prefix)
R2_LUTS_BUCKET = (os.getenv("R2_LUTS_BUCKET", "").strip() or R2_BUCKET).strip()
# Default prefix matches your provided path "LUTs Previews/"
R2_LUTS_PREFIX = os.getenv("R2_LUTS_PREFIX", "LUTs Previews/").lstrip("/")

MAX_FILES = int(os.getenv("MAX_FILES", "100"))

# Payments (Dodo)
DODO_API_BASE = os.getenv("DODO_API_BASE", "https://api.dodo-payments.example").rstrip("/")
DODO_CHECKOUT_PATH = os.getenv("DODO_CHECKOUT_PATH", "/checkout/sessions").strip()
