...
def compute_dof(f_mm: float, N: float, coc_mm: float, focus_distance_m: float) -> Dict[str, Optional[float]]:
    # Convert to mm
    ...
    # JSON-safe values: replace infinities with None
    far_m = None if math.isinf(far) else (far / 1000.0)
    total_m = None if math.isinf(total) else (total / 1000.0)
